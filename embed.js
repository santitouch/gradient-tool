/* Embeddable MonopoGradient runtime
   Usage:
     MonopoGradient.mount("#el", {
...
(function () {
  ...
  function mount(selector, config) {
    ...
    // Host styling defaults (non-destructive)
    ...
    const fragSrc = `
      ...
      void main(){
        ...
        // STATIC grain: no time, pinned to pixels
    ...
    // Mouse in local host coords (smoothed)
    const mouse = { x: 0.5, y: 0.5 };
    const mouseT = { x: 0.5, y: 0.5 };

    host.addEventListener("mousemove", (e) => {
      const r = host.getBoundingClientRect();
      mouseT.x = clamp01((e.clientX - r.left) / Math.max(1, r.width));
      mouseT.y = clamp01(1.0 - (e.clientY - r.top) / Math.max(1, r.height));
    });
    ...
    // Colors resolved once at mount time (fast).
    // If you want dynamic runtime updates, call instance.setConfig().
    function getColors() {
      const a = cfg.colors || [];
      const [r1,g1,b1] = hexToRgb01(a[0] || "#223BFF");
      const [r2,g2,b2] = hexToRgb01(a[1] || "#FF6A2B");
      const [r3,g3,b3] = hexToRgb01(a[2] || "#0B0B10");
      const [r4,g4,b4] = hexToRgb01(a[3] || "#9CB6C7");
    ...
    function draw(ms) {
      ...
      gl.uniform3f(loc.c1, colors.r1, colors.g1, colors.b1);
      gl.uniform3f(loc.c2, colors.r2, colors.g2, colors.b2);
      gl.uniform3f(loc.c3, colors.r3, colors.g3, colors.b3);
      gl.uniform3f(loc.c4, colors.r4, colors.g4, colors.b4);
      ...
      gl.uniform1f(loc.u_mouseRadius, Number(cfg.mouseRadius));
